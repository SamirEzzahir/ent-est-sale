...
const adminBase = () => resolveApiBase(import.meta.env.VITE_ADMIN_API_URL as string | undefined, '/api/admin')
...
export async function listAdminUsers(): Promise<BackendAdminUser[]> {
  ...
  return text ? (JSON.parse(text) as BackendAdminUser[]) : []
}

export async function adminCreateUserRequest(payload: AdminCreateUserPayload): Promise<BackendAdminUser> {
  ...
  return JSON.parse(text) as BackendAdminUser
}

export async function updateAdminUserRole(userId: string, role: AppRealmRole): Promise<BackendAdminUser> {
  const response = await fetch(`${adminBase()}/users/${encodeURIComponent(userId)}/roles`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      ...authHeader(),
...
export async function updateAdminUser(userId: string, payload: AdminUpdateUserPayload): Promise<BackendAdminUser> {
  const body = {
    username: payload.username.trim(),
    role: payload.role,
  ...
  const response = await fetch(`${adminBase()}/users/${encodeURIComponent(userId)}`, {
    method: 'PATCH',
    headers: {
      'Content-Type': 'application/json',
      ...authHeader(),
