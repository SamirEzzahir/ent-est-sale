...
function AccountApprovalsRedirect() {
  const { rolePrefix } = useParams()
  const target = `/${rolePrefix ?? 'admin'}/users`
  return <Navigate to={target} replace />
}

function LegacyAdminUsersRedirect() {
  const { rolePrefix } = useParams()
  const target = `/${rolePrefix ?? 'admin'}/users`
  return <Navigate to={target} replace />
}
...
export const router = createBrowserRouter([
  ...
  { path: '/help', element: <HelpPage /> },
  {
    ...
    children: [
      ...
      { path: 'admin/users', element: <LegacyAdminUsersRedirect /> },
      { path: 'account-approvals', element: <AccountApprovalsRedirect /> },
